define([], function () {
  ...
  Handlebars.registerHelper("ul_list", function (v1, v2, options) {
    var dom = "<ul>"+
        "{{#each objects}}"+
        "<li>"+
        "<p><a class='plan-main-item' href='pages/monitor/infodetail.html?docID={{this.docId}}' data-pageturn='true'>{{this.title}}</a>"+
        "{{#if this.sentiment}}"+
        ...
        "{{/if}}"+
        "</p>"+
        "<span class='plan-main-item-like'>{{this.pubTime}} [<a href='pages/similarArticle.html?docID={{this.docId}}' data-pageturn='true'> 相似文章{{this.similarCount}} </a>]</span>"+
